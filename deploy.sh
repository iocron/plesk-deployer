...
if [[ $PLESK_MODSECURITY_FIREWALL == 1 ]]; then
	if plesk bin server_pref --show-web-app-firewall | grep -q "${PLESK_MODSECURITY_FIREWALL_RULESET}"; then
		...
		plesk bin server_pref --update-web-app-firewall -waf-rule-engine on -waf-rule-set $PLESK_MODSECURITY_FIREWALL_RULESET -waf-rule-set-update-period daily -waf-config-preset fast | tee -a $LOG_DEPLOYMENT;
